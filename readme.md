...
To get a snapshot
```sh
$ curator --config config.yml snapshot.yml
```
To remove a indices
```sh
$ curator --config config.yml delete.yml
```

# How should be define cron job on production ?
```sh
$ crontab -e
# daily
* * * * * sh $HOME/elk-manager/scripts/snapshot.sh && sh $HOME/elk-manager/scripts/delete.sh
```

# How monitoring on machine ?
```sh
...
```sh
GET http://localhost:9200/_snapshot/tragate-logs/_all?pretty 
```
